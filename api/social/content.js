...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'POST' && req.body.action === 'wiki') {
      ...
      const { type, title, content, slug, categoryId, order = 0, isPublished = true } = req.body

      // 创建分类
      if (type === 'create-category') {
        const { name, slug: catSlug, description = '', order: catOrder = 0, isVisible = true } = req.body
        ...
        // 检查分类标识符是否重复
        const existingCategory = await wikiCategories.findOne({ slug: catSlug })
        if (existingCategory) {
      ...
      if (type === 'create-document') {
        if (!title || !content || !slug) {
          return res.status(400).json({ success: false, message: '标题、内容和标识符不能为空' })
        }

        // 检查文档标识符是否重复
        const existingDoc = await wikis.findOne({ slug })
        if (existingDoc) {
        ...
        const newDoc = {
          ...
          author: {
            id: decoded.userId,
            username: currentUser.username,
            displayName: currentUser.profile?.displayName || currentUser.profile?.nickname || currentUser.username
          },
          createdAt: new Date(),
          updatedAt: new Date()
        }
    ...
    // 更新 Wiki 文档/分类
    if (req.method === 'PUT' && req.body.action === 'wiki') {
      if (currentUser.role !== 'admin') {
        return res.status(403).json({ success: false, message: '需要管理员权限' })
      }
      ...
      if (type === 'update-category') {
        const { name, slug: catSlug, description, order: catOrder, isVisible } = req.body
        ...
        if (catSlug !== undefined) updateData.slug = catSlug
      ...
      if (type === 'update-document') {
        const { title, content, slug, categoryId, order, isPublished } = req.body
    ...
    // 删除 Wiki 文档/分类
    if (req.method === 'DELETE' && req.query.action === 'wiki') {
      if (currentUser.role !== 'admin') {
        return res.status(403).json({ success: false, message: '需要管理员权限' })
      }
